...
from tensorflow.keras.layers import CuDNNLSTM, Bidirectional, Dropout, Dense, Activation, Flatten, Conv1D, Reshape, \
    MaxPool1D
...
class UseANN(object):
    ...
    def __use_BiLSTM(self):
        ...
        model = Sequential()
        model.add(Reshape(input_shape=(self.train.shape[1], self.train.shape[2]),target_shape=(self.train.shape[1], self.train.shape[2])))
        #model.add(Dense(units=64,kernel_constraint=tensorflow.keras.constraints.MinMaxNorm(min_value=0.0, max_value=1.0, rate=0.8, axis=1),kernel_regularizer=tf.keras.regularizers.l2(0.001),activity_regularizer=tf.keras.regularizers.l1(0.001))) 
    ...
    def plt(self,model_type,filename='./', e=2, b=32, l=2e-4, beta=0.9):
        self.e = e
        self.b = b
        self.l = l
        self.beta = beta
        ...
        adam = Adam()
        self.model.compile(optimizer=adam, loss='binary_crossentropy', metrics=['accuracy'], )
        plot_model(self.model, to_file= filename +self.model_type + '.png', show_shapes=True, dpi=600)
...
def use_fold_ANN(model_type_list=[],param={},number=[1], train_dir='./', label_dir='./', ctl='CLS'):
    plt_i = 1
    for i in number:
        Train_ANN = UseANN(i)

        # 数据的读取
        train_file = train_dir + 'fold' + str(i) + '/train.txt'
        label_file = label_dir + 'fold' + str(i) + '/train.txt'

        test_file = train_dir + 'fold' + str(i) + '/test.txt'
        test_label_file = label_dir + 'fold' + str(i) + '/test.txt'

        Train_ANN.get_CLS_data(train_file, label_file)  
        ...
        if plt_i == 1:
            for model_type in model_type_list:
                # Train_ANN.plt(model_type=model_type, filename='./')
                pass
            plt_i +=1

        param_EI = use_grid_ANN(Train_ANN,model_type_list,param)
        param_EI = pd.DataFrame(param_EI,columns=None,index=None)
        param_EI.to_csv('./neuropeptides/aac/fold' + str(i) + '.csv', header=False,index=False)
...
if __name__ == '__main__':
    ...
    param = {
        'e': [64],
        'b': [16, 32, 64],
        'l': [0.00002, 0.0002, 0.002],
        'beta': [0.9],
    }
    
    # bertfea/AMP/neuropeptides/train
    train_dir = '/neuropeptides/combin_fea/neuropeptides/aac/'
    label_dir = '/neuropeptides/combin_fea/neuropeptides/aac/'  # 必须以 / 结尾
   

    use_fold_ANN(model_type_list=model_list, param=param, number=number, train_dir=train_dir, label_dir=label_dir)
